...
class ClassifierMLP2(nn.Module):
...
class extractor(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv_1 = nn.Conv1d(1, 32, kernel_size=15, stride=1, padding='same')
        self.bn_1 = nn.BatchNorm1d(32)
        self.relu_1 = nn.ReLU(inplace=True)
        self.pool_1 = nn.MaxPool1d(kernel_size=2, stride=2)

        self.conv_2 = nn.Conv1d(32, 64, kernel_size=7, stride=1, padding='same')
        self.bn_2 = nn.BatchNorm1d(64)
        self.relu_2 = nn.ReLU(inplace=True)
        self.pool_2 = nn.MaxPool1d(kernel_size=2, stride=2)

        self.conv_3 = nn.Conv1d(64, 64, kernel_size=5, stride=1, padding='same')
        ...
        self.linear = nn.Linear(64 * 256, 512)
...
class Trainset(InitTrain):
    ...
    def __init__(self, args):
        ...
        output_size = 512# 
        self.ps_1 = extractor().to(self.device)
        self.ps_2 = extractor().to(self.device)
        self.pt = extractor().to(self.device)
        self.sh = extractor().to(self.device)
        self.C = ClassifierMLP2(input_size=output_size, output_size=args.num_classes,
                                          dropout=args.dropout, last=None).to(self.device)
        self.de = Shared_feature_decoder().to(self.device)
        self.domain_discri = ClassifierMLP2(input_size=output_size, output_size=1,
