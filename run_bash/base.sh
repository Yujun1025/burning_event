...
#   done
# done
declare -a models=("MFSAN")
# Loop over each model for CWRU dataset as source and JNU as target with multi_source mode
# for model in "${models[@]}"; do
#   for source in "${CWRU_sources[@]}"; do
#     for target in "${JNU_targets[@]}"; do
#       python train.py --model_name="$model" --source="$source" --target="$target" --train_mode multi_source --project="experiment" --random_state 5827
#       python train.py --model_name="$model" --source="$source" --target="$target" --train_mode multi_source --project="experiment" --random_state 2463
#       python train.py --model_name="$model" --source="$source" --target="$target" --train_mode multi_source --project="experiment" --random_state 927
#       python train.py --model_name="$model" --source="$source" --target="$target" --train_mode multi_source --project="experiment" --random_state 1025
#       python train.py --model_name="$model" --source="$source" --target="$target" --train_mode multi_source --project="experiment" --random_state 2109
#     done
#   done
# done
# # Loop over each model for JNU dataset as source and CWRU as target with multi_source mode
# for model in "${models[@]}"; do
#   for source in "${JNU_sources[@]}"; do
#     for target in "${CWRU_targets[@]}"; do
#       python train.py --model_name="$model" --source="$source" --target="$target" --train_mode multi_source --project="experiment" --random_state 5827
