...
class DomainAdversarialLoss(nn.Module):
    ...
    def forward(self, f_s, f_t, w_s = None, w_t = None):
        ...
        # print(d_s.shape)
        # print(d_label_s.shape)
        # loss = 0.5 * (self.bce(d_s, d_label_s, w_s.view_as(d_s)) + \
        #                             self.bce(d_t, d_label_t, w_t.view_as(d_t)))
        loss = 0.5 * (F.binary_cross_entropy(d_s, d_label_s) + \
                                    F.binary_cross_entropy(d_t, d_label_t))
        return loss, d_accuracy

class dfn_da(nn.Module):

    def __init__(self, domain_discriminator, reduction = 'mean', grl = None):
        super(dfn_da, self).__init__()
        self.grl = WarmStartGradientReverseLayer(alpha=1., lo=0., hi=1.,
                            max_iters=1000, auto_step=True) if grl is None else grl
        self.domain_discriminator = domain_discriminator
        self.bce = lambda input, target, weight: \
    ...
    def forward(self, f_s1, f_s2, f_t, w_s = None, w_t = None):
        ...
        f_s = torch.cat((f_s1, f_s2), dim = 0)
        d_s = torch.cat((d_s1, d_s2), dim = 0)
        d_label_s = torch.ones((f_s.size(0), 1)).to(f_s.device)
        d_label_t = torch.zeros((f_t.size(0), 1)).to(f_t.device)
        # ???
        d_accuracy = (2 / 3) * (binary_accuracy(d_s, d_label_s)) +\
                     (1 / 3) * (binary_accuracy(d_t, d_label_t))
        ...
        loss = 0.5 * (F.binary_cross_entropy(d_s, d_label_s) + \
                                    F.binary_cross_entropy(d_t, d_label_t))
        return loss, d_s1, d_s2, d_t, d_accuracy
    
class new_da(nn.Module):

    def __init__(self, domain_discriminator, reduction = 'mean', grl = None):
        super(new_da, self).__init__()
        self.grl = WarmStartGradientReverseLayer(alpha=1., lo=0., hi=1.,
                            max_iters=1000, auto_step=True) if grl is None else grl
        self.domain_discriminator = domain_discriminator
        self.bce = lambda input, target, weight: \
